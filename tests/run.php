 ...
 *
 * Assertions come from Kumwe\Context\Tests\TestCase. No framework, so the suite runs on any supported PHP with no
...
spl_autoload_register(static function (string $class): void {
    $prefixes = [
        'Kumwe\\Context\\Tests\\' => __DIR__ . '/',
        'Kumwe\\Context\\' => dirname(__DIR__) . '/src/',
    ];
...
if ($files === []) {
    fwrite(STDERR, "Access Context suite failed: no test case files were discovered.\n");
    exit(1);
}
...
foreach ($files as $file) {
    $class = 'Kumwe\\Context\\Tests\\Case\\' . basename($file, '.php');
    if (!class_exists($class)) {
        $failures[] = "{$file} declares no {$class}.";
        continue;
    }
    $case = new $class();
    if (!$case instanceof Kumwe\Context\Tests\TestCase) {
...
echo "\nAccess Context suite passed: {$totalTests} tests, {$totalAssertions} assertions.\n";
