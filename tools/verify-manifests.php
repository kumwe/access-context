...
if ($package !== 'kumwe/access-context' || $namespace !== 'Kumwe\\Context\\') {
    $failures[] = 'composer.json must name kumwe/access-context with the single PSR-4 root Kumwe\\Context\\.';
}
...
foreach ($symbols as $fqcn => $entry) {
    if (!is_string($fqcn) || !str_starts_with($fqcn, 'Kumwe\\Context\\')) {
...
foreach ($entries as $entry) {
    ...
    if (!is_string($id) || preg_match('/^access-context\.[a-z0-9-]+(\.[a-z0-9-]+)*$/', $id) !== 1) {
        $failures[] = 'A capability identifier must be dot-separated and prefixed with access-context.';
        continue;
    }
    if (isset($ids[$id])) {
...
if ($handoff !== false) {
    ...
    $pullRequest = '/^  pull_request: "?https:\/\/github\.com\/kumwe\/access-context\/pull\/[0-9]+"?$/m';
