...
$workspace = rtrim(sys_get_temp_dir(), '/\\') . '/kumwe-access-context-consumer-' . bin2hex(random_bytes(6));
...
consumerRun(
    [
        ...
        '--format=zip',
        '--dir=' . $distDirectory,
        '--file=kumwe-access-context',
    ],
    $workspace,
);
$archive = $distDirectory . '/kumwe-access-context.zip';
if (!is_file($archive)) {
...
$consumerMetadata = [
    ...
    'license' => 'proprietary',
    'require' => ['kumwe/access-context' => $release],
    'repositories' => [
...
$installed = $consumer . '/vendor/kumwe/access-context';
...
foreach (array_keys($classmap) as $symbol) {
    if (
        ...
        || str_starts_with($symbol, 'Kumwe\\Context\\Tests\\')
    ) {
